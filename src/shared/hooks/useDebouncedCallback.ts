...
export function useDebouncedCallback<T extends (...args: any) => ReturnType<T>>(
  ...
  const lastCallTime: any = useRef(null);
  ...
  const debounced = useMemo(() => {
    ...
    const startTimer = (pendingFunc: () => void, _wait: number) => {
      if (useRAF) cancelAnimationFrame(timerId.current);
      timerId.current = useRAF
        ? requestAnimationFrame(pendingFunc)
        : setTimeout(pendingFunc, _wait);
    };
    ...
    const shouldInvoke = (time: number) => {
      ...
      return (
        ...
        timeSinceLastCall < 0 ||
        (maxWait && maxing && timeSinceLastInvoke >= maxWait)
      );
    };
    ...
    const timerExpired = () => {
      ...
      const remainingWait =
        maxing && maxWait ? Math.min(timeWaiting, maxWait - timeSinceLastInvoke) : timeWaiting;
