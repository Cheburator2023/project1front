...
import { observeRect, getScrollbarSize } from './helpers';
...
export interface TableProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Колбек, который срабатывает после завершения перетаскивания столбца таблицы на новое место.
   * columnName - name столбца, который был перетащен;
   * nextColumnName - name столбца, перед которым встал перетащенный столбец.
...
export const Table: React.FC<TableProps> = ({
  ...
  const columnMinWidth =
    dimension === 's' || dimension === 'm' ? COLUMN_MIN_WIDTH_M : COLUMN_MIN_WIDTH_L;
  ...
  const isAnyStickyColumnDraggable =
    columnList.filter((col) => col.sticky && col.draggable).length > 0;
  ...
  const zebraRows = greyZebraRows
    ? tableRows.reduce<ZebraRows>((acc: ZebraRows, row: TableRow, index: number) => {
        if (rowToGroupMap[row.id]) {
          const indexInGroup = groupToRowsMap[rowToGroupMap[row.id].groupId].rows.indexOf(
            String(row.id),
          );
  ...
  React.useEffect(() => {
    ...
    function handleDrop(item: HTMLElement | null, before: HTMLElement | null) {
      ...
      if (columnName) {
        if (stickyCols?.contains(item) && before === null) {
          ...
          const nextColumnName = before?.dataset?.thColumn ?? null;
          dragInfo.current = { columnName, nextColumnName };
          columnDragCallback.current?.(columnName, nextColumnName);
        }
      }
    }
    ...
    function handleDragEnd() {
      if (dragInfo.current) {
        onColumnDragEnd?.(dragInfo.current.columnName, dragInfo.current.nextColumnName);
  ...
  const parentGroupWillBeChecked = (changedDepId: RowId) => {
    ...
    const value = groupInfo?.rows.some((rowId) =>
      rowId === changedDepId.toString()
        ? !rowToGroupMap[rowId].checked
        : rowToGroupMap[rowId].checked,
    );
  ...
  const renderGroupRow = (row: TableRow) => {
    ...
    const checked = hasGroupRows
      ? row.groupRows?.every((rowId) => rowToGroupMap[rowId].checked)
      : row.selected;
  ...
  const renderRegularRow = (row: TableRow, idx: number, columns: Column[]) => (
    <RegularRow
      row={row}
      dimension={dimension}
      checkboxDimension={checkboxDimension}
      columns={columns}
      stickyColumns={[...columns].filter((col) => col.sticky)}
      displayRowExpansionColumn={displayRowExpansionColumn}
      displayRowSelectionColumn={displayRowSelectionColumn}
  ...
  const renderRow = (row: TableRow, index: number, columns: Column[]) => {
    const isGroupRow = !!groupToRowsMap[row.id];
    const rowInGroup = !!rowToGroupMap[row.id];
    ...
    const node = (isGroupRow || visible) && (
      <RowWrapper
        ...
        key={`row_${row.id}`}
      >
        {isGroupRow ? renderGroupRow(row) : renderRegularRow(row, index, columns)}
      </RowWrapper>
    );
  ...
  const renderBody = () => {
    ...
    return virtualScroll ? (
      ...
      <ScrollTableBody ref={scrollBodyRef} className="tbody">
        {tableRows.map((row, index) => renderRow(row, index, columnList))}
      </ScrollTableBody>
    );
  };
      ...
      <HiddenHeader ref={hiddenHeaderRef} data-verticalscroll={verticalScroll}>
        ...
        >
          ...
          {columnList.map((col, index) =>
            col.sticky ? null : renderHeaderCell(col as Column, index),
          )}
        </HeaderCellsWrapper>
      </HiddenHeader>
    );
              ...
              {displayRowSelectionColumn && (
                ...
                >
                  <Checkbox
                    ...
                    indeterminate={
                      (someRowsChecked && !allRowsChecked) || headerCheckboxIndeterminate
                    }
